 ...
 */
 ...
 (function($) {
    ...
    $.richFilemanagerPlugin = function(element, pluginOptions)
    {
        ...
        // return filename without extension
        var getPureFilename = function(filename) {
            if(filename.lastIndexOf('.') !== -1) {
      ...
      var getFilteredFileExtensions = function() {
        ...
        var selectItem = function(resourceObject) {
            ...
            if(typeof parent.tinyMCE !== "undefined" && parent.tinyMCE.majorVersion === "5") {
                if(!resourceObject) {
                    fm.error("Selected File Error");
                    return
                }
    
                var insertFileType = _url_.param('filter');
                var fileName = getPureFilename(resourceObject.id);
                var fileExt = getExtension(resourceObject.id);
    
                if(insertFileType === "image" && !isImageFile(resourceObject.id)) {
                    fm.error("Please select image type file.");
                    return
                } else if(insertFileType === "media" && (!isVideoFile(resourceObject.id) || !isAudioFile(resourceObject.id))) {
                    fm.error("Please select media type file.");
                    return
                }
    
                parent.postMessage({mceAction: 'FileSelected', content: previewUrl, fileName: fileName, fileExt: fileExt});
                setTimeout(function() {
                    parent.tinyMCE.activeEditor.windowManager.close();
                }, 300);
            }
